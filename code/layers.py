...
class GraphAttentionBiLSTMConvolution():
    ...
    def __call__(self, h, adj, training=True):
        with tf.compat.v1.name_scope(self.name):
            ...
            # Re-attach static rank/feature shape lost by tf.cond in TF1 graph mode.
            h_node = tf.ensure_shape(h_node, [None, self.output_dim])
            ...
            h_prime = tf.ensure_shape(h_prime, [None, self.output_dim])
...
class GraphAttentionConvolution():
    ...
    def __call__(self, h, adj, training=True):
        with tf.compat.v1.name_scope(self.name):
            ...
            Wh = tf.ensure_shape(Wh, [None, self.output_dim])
            e = self._prepare_attentional_mechanism_input(Wh)
            zero_vec = -9e15 * tf.ones_like(e)
            ...
            h_prime = tf.matmul(attention, Wh)
            h_prime = tf.ensure_shape(h_prime, [None, self.output_dim])
        return tf.ensure_shape(self.leakyrelu(h_prime) + self.former, [None, self.output_dim])
...
class GraphBERTLayer(tf.keras.layers.Layer):
    def __init__(self, embed_dim, num_heads=4, dropout_rate=0.1, name=None):
        ...
        self.key_dim = embed_dim // num_heads if embed_dim % num_heads == 0 else max(1, embed_dim // num_heads)
    ...
    def call(self, x, training=False):
        ...
        attn_output = self.att(x_exp, x_exp, training=training)
        ...
        def squeeze():
            return tf.squeeze(out2, axis=0)

        def passthrough():
            return out2

        return tf.cond(tf.equal(rank, 2), squeeze, passthrough)
